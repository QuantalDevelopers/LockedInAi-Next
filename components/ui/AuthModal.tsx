...
export default function AuthModal({ isOpen, onClose, redirectUrl, companyId }: AuthModalProps) {
  const [email, setEmail] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    //   setFadeOut(true);
    //   setTimeout(() => {
    //     onClose(); // or do any post‐submit action
    //   }, 300);
    // } catch (error: any) {
    //   toast({
    //     variant: "destructive",
      ...
      >
        <div className="w-full flex items-center justify-center p-2">
          ...
          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="flex items-center justify-center w-full border border-gray-300 rounded-md py-2 text-gray-700 hover:bg-gray-50 mb-4 bg-white"
          >
            {/* Google Icon */}
            <svg
              className="mr-2"
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEmail(e.target.value)}
                required
                className="text-white bg-[rgb(30,30,30)] mt-1 w-full"
              />
            </div>
