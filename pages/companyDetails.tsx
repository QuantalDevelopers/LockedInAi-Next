...
// // const CompanyDetailsContent = ({ companyId }: { companyId: string }) => {
// //   const { user, isLoaded, isSignedIn } = useUser();  // Clerk hook for user info
...
// //       const redirectUrl = `https://app.lockedinai.com/sign-in?redirect_url=localhost:3000/companyDetails/${companyId}`;
// //       redirectToSignIn({ redirectUrl });
// //     }
// //   }, [isLoaded, isSignedIn, companyId, redirectToSignIn]);
...
// //       const { data, error } = await supabase
// //         .from('interviews')
// //         .select(`*, interview_questions (id, question, answer)`)
// //         .eq('company_id', companyId)
// //         .order('created_at', { ascending: false });
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Explore in-depth interview experiences, company details, and reviews on Lockedin Ai. Discover insights from real candidates about company interviews."
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "description":
                "Explore in-depth interview experiences, company details, and reviews on Lockedin Ai. Discover insights from real candidates about company interviews.",
...
const CompanyDetailsContent = ({ companyId }: { companyId: string }) => {
  ...
  return (
    ...
    style={{ backgroundColor: "rgb(30, 30, 30)"}}>
      ...
      <AuthModal isOpen={modalOpen} onClose={handleModalClose} redirectUrl={`/companyDetails/${companyId}`} companyId={`${companyId}`} />
    </>
  );
};
